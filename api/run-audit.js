...
function sanitizeIssueTitle(issue) {
  if (!issue || typeof issue !== 'object') return issue;
  if (isValidTitle(issue.title, issue.guidance)) return issue;
  return { ...issue, title: deriveTitle(issue) };
}
...

{
  "issues": [
    ...
    if (Array.isArray(parsed.issues)) {
      parsed.issues = parsed.issues.map(sanitizeIssueTitle);
    }
